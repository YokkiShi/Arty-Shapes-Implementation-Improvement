/* added a few functions - PLR
 *
 * added hacks to stop program crashing
 * but I don't know what side effects it introduces!
 *
 * July 2002
 */
...
#define EQ(a, b)           (ABS((a) - (b)) <= GPC_EPSILON)
#define NE(a, b)           (ABS((a) - (b)) >  GPC_EPSILON)
...
/* ADDED - PLR
 * make a new polygon from values in arrays
 */
void gpc_create_polygon(int x[], int y[], int no_pixels, gpc_polygon *p)
{
  int c, v;
  ...
  for (c= 0; c < p->num_contours; c++)
  {
    p->contour[c].num_vertices = no_pixels;
...
/* ADDED - PLR
 * make a new polygon from values in double arrays
 */
void gpc_create_polygon2(double x[], double y[], int no_pixels, gpc_polygon *p)
{
  int c, v;
...
/* ADDED - PLR
 * make a new set of polygons from values in arrays
 * each polygon is a triangle (3 vertices)
 */
void gpc_create_polygon3(double x[], double y[], int no_pixels, gpc_polygon *p)
{
  int c, v;

  p->num_contours = no_pixels / 3;

  MALLOC(p->contour, p->num_contours
         * sizeof(gpc_vertex_list), "contour creation");
  for (c= 0; c < p->num_contours; c++)
  {
    p->contour[c].num_vertices = 3;

    MALLOC(p->contour[c].vertex, p->contour[c].num_vertices
           * sizeof(gpc_vertex), "vertex creation");

    for (v= 0; v < p->contour[c].num_vertices; v++) {
      p->contour[c].vertex[v].x = x[3*c+v];
      p->contour[c].vertex[v].y = y[3*c+v];
	}
  }
}
...
/* ADDED - PLR
 * write in pixel format
 * duplicate 1st vertex
 */
void gpc_write_polygon2(FILE *fp, gpc_polygon *p)
{
  int c, v;

  fprintf(fp, "pixel_float\n");
  if (p->num_contours == 0) {
    fprintf(fp, "list: 0\n");
    fprintf(fp, "-1 -1\n");
  }
...
/* ADDED - PLR
 * write in pixel format
 * DON'T duplicate 1st vertex
 */
void gpc_write_polygon3(FILE *fp, gpc_polygon *p)
{
  int c, v;
